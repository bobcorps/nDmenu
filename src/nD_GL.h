...
SDL_GLContext screen_GLCONTEXT = NULL;
...
texture_STRUCT *load_texture(
  ...
  SDL_Surface *temp = SDL_CreateRGBSurface(SDL_SWSURFACE, texture_w, texture_h,
  ...
  // copy the source pixels verbatim, alpha included, rather than blending
...
//===========================================================================================================================
texture_STRUCT *load_texture_from_non_square_surface(
    SDL_Surface *
        surface) { //===========================================================================================================================
...
//===========================================================================================================================
texture_STRUCT *load_texture_directly_from_square_surface(
    SDL_Surface *
        temp) { //===========================================================================================================================
...
//===========================================================================================================================
void draw_texture(
    texture_STRUCT *sprite_TEXTURE, int x,
    int y) { //===========================================================================================================================
...
//===========================================================================================================================
void draw_texture_alpha(
    texture_STRUCT *sprite_TEXTURE, int x, int y,
    float
        a) { //===========================================================================================================================
  ...
  GLfloat col[] = {1, 1, 1, a, 1, 1, 1, a, 1, 1, 1, a, 1, 1, 1, a};
...
void fill_rect_gradient(
    int x, int y, int w, int h, float r, float g, float b,
    float
        a) { //===========================================================================================================================
             /*glDisable(GL_TEXTURE_2D);
  ...
  GLfloat col[] = {0, 0, 0, 1, 0, 0, 0, 1, r, g, b, a, r, g, b, a};
...
void fill_rect(
  ...
  GLfloat col[] = {r, g, b, a, r, g, b, a, r, g, b, a, r, g, b, a};
...
void init_gl() { //===========================================================================================================================
...
#ifdef BUILD_WIZ
  screen_WINDOW = SDL_CreateWindow("nD", SDL_WINDOWPOS_CENTERED,
                                   SDL_WINDOWPOS_CENTERED, 320, 240, 0);
#else
  // windows GLES emulation
  screen_WINDOW =
      SDL_CreateWindow("nD", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
  ...
  screen_GLCONTEXT = SDL_GL_CreateContext(screen_WINDOW);
#endif

  SDL_ShowCursor(0);

  // SDL_SetWindowIcon(screen_WINDOW, SDL_LoadBMP("icon.bmp"));
  ...
  glSurface =
      eglCreateWindowSurface(glDisplay, glConfig, hNativeWnd, attrib_list);

  glContext =
      eglCreateContext(glDisplay, glConfig, EGL_NO_CONTEXT, attrib_list);
  ...
  if (framebuffer) {
    glBindTexture(GL_TEXTURE_2D,
                  screen_TEXTURE->texture_id); // modify (or draw) our texture

    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 320, 240, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, 0); // the texture is w x h pixels

    // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,GL_LINEAR);
    // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,GL_LINEAR); //no
    // mipmaps (see below)

    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                    GL_CLAMP_TO_EDGE); // no repeat (see below)
    ...
    // You can render pixels into a texture using a framebuffer object.  Here's
    // how you make a framebuffer object "fb" hooked up to render into "myTex":
    glGenFramebuffersEXT(1, &fb);
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, fb);

    glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
                              GL_TEXTURE_2D, screen_TEXTURE->texture_id, 0);
  }
...
#else
  if (framebuffer) {
    // Be sure to reset the rendering state afterwards by binding the zero
    // framebuffer object (the screen):
...
#else
  SDL_GL_DeleteContext(screen_GLCONTEXT);
  screen_GLCONTEXT = NULL;
#endif
