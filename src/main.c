...
//===========================================================================================================================
// smpeg2 hands back a YV12 frame rather than drawing into a surface for us,
// so unpack it into the surface the rest of the movie pipeline expects.
//===========================================================================================================================
static void movie_display_callback(void *data, SMPEG_Frame *frame)
{
    SDL_Surface *dst = (SDL_Surface *)data;

    if (dst == NULL || frame == NULL || frame->image == NULL)
        return;

    if (SDL_MUSTLOCK(dst))
        SDL_LockSurface(dst);

    SDL_ConvertPixels(frame->w, frame->h,
                      SDL_PIXELFORMAT_YV12, frame->image, frame->image_width,
                      dst->format->format, dst->pixels, dst->pitch);
...
{ //===========================================================================================================================
    if (dir == CLOCKWISE)
    {
        ...
        selected_wheel_item++;
        if (selected_wheel_item >= loaded_wheel_items)
            selected_wheel_item = 0;
        wheel_sound_queue++;
    }
    else if (dir == COUNTERCLOCKWISE)
    {
        ...
        selected_wheel_item--;
        if (selected_wheel_item < 0)
            selected_wheel_item = loaded_wheel_items - 1;
        wheel_sound_queue++;
    }
}
...
{ //===========================================================================================================================
    ...
    while (SDL_PollEvent(&event_EVENT))
    {
        ...
        if (event_EVENT.type == SDL_KEYDOWN)
        {
            ...
            if (event_EVENT.key.keysym.sym == SDLK_ESCAPE)
                quit = 1;
            if (event_EVENT.key.keysym.sym == SDLK_RETURN)
                action_pressed = 1;
        }
        ...
        if (event_EVENT.type == SDL_JOYBUTTONDOWN)
        {
            ...
            if (event_EVENT.jbutton.button == GP2X_BUTTON_Y)
                quit = 1;
            if (event_EVENT.jbutton.button == GP2X_BUTTON_B)
                action_pressed = 1;
        }
...
{ ///===========================================================================================================================
    ...
    // for(m=0;m<4;m++)
    {
        movie_YUV_SURFACE[m] = SDL_CreateRGBSurface(SDL_SWSURFACE, movieInfo[m].width, movieInfo[m].height, 32, rmask, gmask, bmask, amask);
        SMPEG_setdisplay(movie[m], movie_display_callback, movie_YUV_SURFACE[m], NULL);
    }
...
//------------------------------------------
        ...
        {
            if (bg_glow_frame[f] != 0)
                draw_texture(bg_glow_TEXTURE[bg_glow_frame[f] - 1], bg_offset.x + bg_glow_offset[f].x, bg_offset.y + bg_glow_offset[f].y);
        }
        //------------------------------------------
        ...
        for (f = 0; f < bg_glow_amt; f++)
        {
            ...
            if (current_ticks > bg_glow_ticks[f] + 20) //+bg_glow_timer[f])
            {
                ...
                if (bg_glow_started[f] == 0)
                {
                    ...
                    if (bg_glow_frame[p] > 20 / bg_glow_amt)
                        bg_glow_started[f] = 1;
                }
                ...
                if (bg_glow_started[f] == 1)
                {
                    ...
                    if (bg_glow_frame[f] >= 20)
                    {
                        ...
                        if (rand() % 2 == 0)
                        {
                            bg_glow_offset[f].y = bg_glow_offset[p].y + 32;
                            bg_glow_offset[f].x = bg_glow_offset[p].x;
                        } // down
                        else
                        {
                            bg_glow_offset[f].x = bg_glow_offset[p].x + 32;
                            bg_glow_offset[f].y = bg_glow_offset[p].y;
                        } // right
                        // bg_glow_offset[f].y=bg_glow_offset[p].y-32;//up
                        // bg_glow_offset[f].x=bg_glow_offset[p].x-32;//left

                        // wrap around
                        if (bg_glow_offset[f].x > 32 * 12)
                            bg_glow_offset[f].x = 0;
                        if (bg_glow_offset[f].x < 0)
                            bg_glow_offset[f].x = 32 * 12;
                        if (bg_glow_offset[f].y > 32 * 10)
                            bg_glow_offset[f].y = 0;
                        if (bg_glow_offset[f].y < 0)
                            bg_glow_offset[f].y = 32 * 10;
        ...
        if (current_ticks > bg_ticks + 20) // 10
        {
            ...
            if (bg_offset.x <= -64 || bg_offset.y <= -64)
            {
                ...
                for (f = 0; f < bg_glow_amt; f++)
                {
                    ...
                    if (bg_glow_offset[f].x < 0)
                        bg_glow_offset[f].x += 12 * 32;
                    if (bg_glow_offset[f].y < 0)
                        bg_glow_offset[f].y += 10 * 32;
                }
            }
        }
        ...
        if (current_ticks > movie_play_ticks)
        {
            ...
            if (movie_render_step == 0 && movie_x == 0 && movie_y == 0) // 240-movie_YUV_SURFACE[movie_selected]->h)
            {
                ...
                if (movie_render_step == 1)
                {
                    ...
                    SDL_SetSurfaceBlendMode(movie_YUV_SURFACE[movie_selected], SDL_BLENDMODE_NONE);
                    ...
                    if (movie_render_step == 2)
                    {
                        ...
                        {
                            movieswapframe = 1;
                            movieswapframe_opposite = 0;
                        }
                        else
                        {
                            movieswapframe = 0;
                            movieswapframe_opposite = 1;
                        }

                        movie_TEXTURE[movieswapframe] = load_texture_directly_from_square_surface(movie_RGBA_SURFACE);
                        if (movie_TEXTURE[movieswapframe_opposite] != NULL)
                            delete_texture(movie_TEXTURE[movieswapframe_opposite]);
        ...
        if (current_ticks >= movie_border_ticks + 100)
        {
            ...
            movie_border_frame++;
            if (movie_border_frame > 12)
                movie_border_frame = 0;
        }
...
//------------------------------------------
            ...
            for (wheel_speed_count = 0; wheel_speed_count <= wheel_speed_limit * (distance_multiplier * df); wheel_speed_count++)
            {
                ...
                if (wheel_item[selected_wheel_item].y + (cart_label_height / 2) != SCREEN_SIZE_Y / 2)
                {
                    ...
                    for (c = 0; c < loaded_wheel_items; c++)
                    {
                        if (wheel_item[c].y > bottomy)
                        {
                            bottomy = wheel_item[c].y;
                            bottomc = c;
                        }
                        if (wheel_item[c].y < topy)
                        {
                            topy = wheel_item[c].y;
                            topc = c;
                        }
                    ...
                    if (wheel_spin_direction == CLOCKWISE)
                    {
                            ...
                            wheel_item[topc].y <= 0 - cart_label_height)
                        {
                            ...
                            if (wheel_item[bottomc].y + cart_label_height >= SCREEN_SIZE_Y)
                                ...
                                wheel_item[topc].y = SCREEN_SIZE_Y;
                        }
                    }
                    ...
                    if (wheel_spin_direction == COUNTERCLOCKWISE)
                    {
                            ...
                            wheel_item[bottomc].y >= SCREEN_SIZE_Y)
                        {
                            if (wheel_item[topc].y - cart_label_height <= 0 - cart_label_height)
                                ...
                                wheel_item[bottomc].y = 0 - cart_label_height;
                        }
                    }
                }
                else
        ...
        if (current_ticks > infopanel_scroll_ticks)
        {
            ...
            if (direction_button_pressed == 0)
            {
                ...
                if (infopanel_x < 0)
                    infopanel_x += 16;
                if (infopanel_y > 240 - infopanel_TEXTURE->image_h)
                    infopanel_y -= 16;
                if (infopanel_x > 0)
                    infopanel_x = 0;
                if (infopanel_y < 240 - infopanel_TEXTURE->image_h)
                    infopanel_y = 240 - infopanel_TEXTURE->image_h;
            }
        }
        ...
        if (current_ticks >= infopanel_border_ticks + 150)
        {
            ...
            if (infopanel_border_frame > 7)
                infopanel_border_frame = 0;
        }
        ...
        if (text_Game_SURFACE != NULL)
        {
            SDL_FreeSurface(text_Game_SURFACE);
            text_Game_SURFACE = NULL;
        }
        if (text_Author_SURFACE != NULL)
        {
            SDL_FreeSurface(text_Author_SURFACE);
            text_Author_SURFACE = NULL;
        }
        if (text_TimePlayed_SURFACE != NULL)
        {
            SDL_FreeSurface(text_TimePlayed_SURFACE);
            text_TimePlayed_SURFACE = NULL;
        }
        if (text_LastPlayed_SURFACE != NULL)
        {
            SDL_FreeSurface(text_LastPlayed_SURFACE);
            text_LastPlayed_SURFACE = NULL;
        }

        if (text_Game_Label_SURFACE != NULL)
        {
            SDL_FreeSurface(text_Game_Label_SURFACE);
            text_Game_Label_SURFACE = NULL;
        }
        if (text_Author_Label_SURFACE != NULL)
        {
            SDL_FreeSurface(text_Author_Label_SURFACE);
            text_Author_Label_SURFACE = NULL;
        }
        if (text_TimePlayed_Label_SURFACE != NULL)
        {
            SDL_FreeSurface(text_TimePlayed_Label_SURFACE);
            text_TimePlayed_Label_SURFACE = NULL;
        }
        if (text_LastPlayed_Label_SURFACE != NULL)
        {
            SDL_FreeSurface(text_LastPlayed_Label_SURFACE);
            text_LastPlayed_Label_SURFACE = NULL;
        }
        ...
        if (text_Game_TEXTURE != NULL)
        {
            delete_texture(text_Game_TEXTURE);
            text_Game_TEXTURE = NULL;
        }
        if (text_Author_TEXTURE != NULL)
        {
            delete_texture(text_Author_TEXTURE);
            text_Author_TEXTURE = NULL;
        }
        if (text_TimePlayed_TEXTURE != NULL)
        {
            delete_texture(text_TimePlayed_TEXTURE);
            text_TimePlayed_TEXTURE = NULL;
        }
        if (text_LastPlayed_TEXTURE != NULL)
        {
            delete_texture(text_LastPlayed_TEXTURE);
            text_LastPlayed_TEXTURE = NULL;
        }

        if (text_Game_Label_TEXTURE != NULL)
        {
            delete_texture(text_Game_Label_TEXTURE);
            text_Game_Label_TEXTURE = NULL;
        }
        if (text_Author_Label_TEXTURE != NULL)
        {
            delete_texture(text_Author_Label_TEXTURE);
            text_Author_Label_TEXTURE = NULL;
        }
        if (text_TimePlayed_Label_TEXTURE != NULL)
        {
            delete_texture(text_TimePlayed_Label_TEXTURE);
            text_TimePlayed_Label_TEXTURE = NULL;
        }
        if (text_LastPlayed_Label_TEXTURE != NULL)
        {
            delete_texture(text_LastPlayed_Label_TEXTURE);
            text_LastPlayed_Label_TEXTURE = NULL;
        }
...
#endif
        {
            ...
            else if (highlight_dir == 0)
            {
                highlight_color += 32;
                if (highlight_color >= 255)
                {
                    highlight_color = 255;
                    highlight_dir = 1;
                }
        ...
        for (c = 0; c < loaded_wheel_items; c++)
        {
            ...
            if (c == selected_wheel_item)
            {
                ...
                if (selected_wheel_item == 1)
                    draw_texture_alpha(wheel_item[c].label_glow_TEXTURE, label_offset.x, label_offset.y, 1. - (float)(highlight_color / 255.));
        ...
        if (selection_box_on == 1)
        {
            ...
            if (current_ticks > selection_box_ticks + 20)
            {
                ...
                selection_box_frame++;
                if (selection_box_frame > 14)
                    selection_box_frame = 0;
            }
        }
    ...
    if (cart_TEXTURE != NULL)
    {
        delete_texture(cart_TEXTURE);
        cart_TEXTURE = NULL;
    }

    if (labelfade_TEXTURE != NULL)
    {
        delete_texture(labelfade_TEXTURE);
        labelfade_TEXTURE = NULL;
    }

    for (t = 0; t < 15; t++)
        if (selection_box_TEXTURE[t] != NULL)
        {
            delete_texture(selection_box_TEXTURE[t]);
            selection_box_TEXTURE[t] = NULL;
        }

    if (bg_TEXTURE != NULL)
    {
        delete_texture(bg_TEXTURE);
        bg_TEXTURE = NULL;
    }

    for (t = 0; t < 10; t++)
        if (bg_glow_TEXTURE[t] != NULL)
        {
            delete_texture(bg_glow_TEXTURE[t]);
            bg_glow_TEXTURE[t] = NULL;
        }

    for (t = 0; t < 13; t++)
        if (movie_border_TEXTURE[t] != NULL)
        {
            delete_texture(movie_border_TEXTURE[t]);
            movie_border_TEXTURE[t] = NULL;
        }

    for (t = 0; t < 2; t++)
        if (movie_TEXTURE[t] != NULL)
        {
            delete_texture(movie_TEXTURE[t]);
            movie_TEXTURE[t] = NULL;
        }

    if (infopanel_TEXTURE != NULL)
    {
        delete_texture(infopanel_TEXTURE);
        infopanel_TEXTURE = NULL;
    }

    for (t = 0; t < 8; t++)
        if (infopanel_border_TEXTURE[t] != NULL)
        {
            delete_texture(infopanel_border_TEXTURE[t]);
            infopanel_border_TEXTURE[t] = NULL;
        }
